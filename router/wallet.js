  ...
  try {
    ...
    records.forEach((record) => {
      ...
      allocations[address] = {
        ...
        baits: parseFloat(record.baits_amount),
      };
    });
...
const getUserAllocation = (userAddress, tokenType) => {
  ...
  if (tokenUpper === "TARAL") return allocation.taral;
  if (tokenUpper === "RVLNG") return allocation.rvlng;
  if (tokenUpper === "BAITS") return allocation.baits; // Add this line
  ...
  try {
    ...
    if (!tokenType || !amount || !transactionHash || !userAddress) {
      return res.status(400).json({
        success: false,
        error:
          "Missing required fields: tokenType, amount, transactionHash, userAddress are required",
      });
    }

    // Validate token type
    if (!["TARAL", "RVLNG", "BAITS"].includes(tokenType.toUpperCase())) {
      return res.status(400).json({
        success: false,
        error: "Invalid token type. Must be TARAL, RVLNG, or BAITS",
      });
    }
    ...
    if (Math.abs(claimAmount - userAllocation) > 0.01) {
    ...
    } else if (tokenType.toUpperCase() === "BAITS") {
      await client.query(
        `UPDATE users SET 
         baits_claimed = baits_claimed + $1,
         total_nova = total_nova + $2,
         last_claim_at = $3,
         updated_at = CURRENT_TIMESTAMP
         WHERE address = $4`,
    ...
    const userTotals = await client.query(
      "SELECT taral_claimed, rvlng_claimed, baits_claimed, total_nova FROM users WHERE address = $1",
      [claimUserAddress]
    );
    ...
    res.status(200).json({
      ...
      data: {
        ...
        userTotals: {
          ...
          baitsClaimed: parseFloat(userStats.baits_claimed),
...
router.get("/allocations/:address", async (req, res) => {
  try {
    ...
    // Get already claimed amounts for ALL tokens
    const claimedTokens = await query(
      "SELECT token_type, amount FROM claims WHERE user_address = $1",
      [address]
    );

    // ✅ Initialize claimed amounts for ALL tokens including BAITS
    const claimed = { 
      taral: 0, 
      rvlng: 0, 
      baits: 0  // ✅ Add BAITS here
    };

    // ✅ Process claimed amounts for ALL token types
    claimedTokens.rows.forEach((claim) => {
      const amount = parseFloat(claim.amount) || 0;
      
      if (claim.token_type === "TARAL") {
        claimed.taral += amount;
      } else if (claim.token_type === "RVLNG") {
        claimed.rvlng += amount;
      } else if (claim.token_type === "BAITS") {  // ✅ Add BAITS processing
        claimed.baits += amount;
      }
    });

    // ✅ Calculate remaining amounts for ALL tokens
    ...
    const baitsRemaining = Math.max(0, allocation.baits - claimed.baits); // ✅ Add BAITS calculation

    // ✅ Return complete allocation data
    res.status(200).json({
      success: true,
      data: {
        ...
        allocations: {
          ...
          baits: {  // ✅ Add complete BAITS allocation object
            allocated: allocation.baits,
            claimed: claimed.baits,
            remaining: baitsRemaining,  // ✅ This should be a number, not null
            canClaim: baitsRemaining > 0,
          },
        },
      },
    });
